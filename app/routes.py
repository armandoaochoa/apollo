...
# Asynchronous function to generate watermarks upon creator submitting recipient info
def watermark(file_share_id, file_handle, file_url):
  # additional variables needed: file_handle, file_url, 
  ...
  recipients = file_share.recipients.all()
  ...
  for recipient in recipients:
    ...
    db.session.commit()
    db.session.close() # do I need this?
  ...
  print('End of watermark!')

# Asynchronous function to generate contract and certificate upon receiving signature from recipient
def generate_contract_and_certificate(ip_address, user_agent, recipient_id, file_share_id, signature_b64_og):
  # arguments to add: signature_b64_og
  ...
  while counter < (use_logs_count + 1): 
    if counter < use_logs_count:
      ...
      if use_logs[counter].user_agent is None:
        ...
        useragent_text = use_logs[counter].user_agent # 'User agent: ' + 
...
def free_send(file_share_parameters):
  ...
  if file_share.watermarked is True:
    ...
    # def watermark() was here...

    # (6/8) Run watermark process asynchronously 
    # just added
    file_share_id = file_share.id
    if app.config['PROD'] == 'production':
      print('Run production redis stuff...')
      #from app import watermark
      job = q.enqueue_call(func=watermark, args=(file_share_id, file_handle, file_url,), result_ttl=5000)
      print('Redis job id: ' + job.get_id())
    else:
      async_watermark_process = Process(target=watermark, args=(file_share_id, file_handle, file_url,), daemon=True)
  ...
  if (recipient.status == 'Verified identity') and (recipient.nda_required is True): 
    if recipient.nda.count() != 1:
      if request.method == 'POST':
        ...
        # def generate_contract_and_certificate()

        recipient_id = recipient.id
        file_share_id = file_share.id

        if app.config['PROD'] == 'production':
          print('Begin Redis task.')
          job = q.enqueue_call(func=generate_contract_and_certificate, args=(ip_address, user_agent, recipient_id, file_share_id, signature_b64_og,), result_ttl=5000)
          print('Redis job id: ' + job.get_id())
        else:
          async_generate_contract_and_certificate_process = Process(target=generate_contract_and_certificate, args=(ip_address, user_agent, recipient_id, file_share_id, signature_b64_og,), daemon=True)
